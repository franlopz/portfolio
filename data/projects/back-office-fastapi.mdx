...
This is the backend for Back Office frontend, it uses the following dependencies:
...
- [ ] Add employees management.

## Setup

1. lnstall and setup MySQL 8.0 or upper.
2. Run `pip install requirements.txt`.
3. rename `env.example` to `.env` and use your own variables.
   1. The variable `SECRET_KEY` is used to encrypt the passwords.
   2. DB_USER, DB_NAME and DB_PASSWORD, are used to connecto to MySQL database.
   3. Variables with `MAIL` prefix are to send and email when user is created.
